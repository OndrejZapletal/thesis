...
*** Structure of Neural Networks
     Neural networks are in general described by three things:
      - model of neuron
      - topology of the network
      - learning algorithm

**** Model of neuron
     Model artificial neuron is shown on figure [[fig:neuron_model][neuron model]].
...
****** Activation Function
       [[Gls:activation function][activation function]] of a neuron gives the [[gls:nn][NN]] the ability to approximate functions. In order to be effective learning algorithm it is necessary that the model is able to approximate the arbitrary function. For this reason the activation function has to be non-linear.
...
Exactly for this reason is each neuron equipped with [[gls:activation function][activation function]] on its output.
       ...
       Sigmoid activation function has the advantage that is differentiable over all of its possible values. Another nicce property is that its otput value is between 1 and 0, which conveniently maps to valid probability distribution.
